...
module.exports.run = (client, message, args, queue, searcher, prefix) => {
    ...
    let prefixes = JSON.parse(fs.readFileSync(path.join(__dirname, "../json/prefixes.json"), "utf-8"));
    ...
    if(newPrefix[1] === undefined){
        ...
        if(newPrefix[1].match(/[\.!\/\\\/\-\?\$\%\&\=\*\+]/)){
            ...
            msg.setDescription('Prefixo válido, o novo prefixo é ' + '`' + `${newPrefix[1]}` + '`');
            ...
            message.channel.send(msg);
            fs.writeFile(path.join(__dirname, "../json/prefixes.json"), JSON.stringify(prefixes), (err) => {
